...
import { ContinueWithGoogle } from "../components/UI/buttons/ContinueWithGoogle";
...
export const Login = () => {
  ...
  useEffect(() => {
    ...
    if (error === "ACCOUNT_EXISTS") {
      setTimeout(() => {
        toast.error("This email is already registered. Please login using password.");
      }, 100);
      // clean URL
    ...
    if (error === "GOOGLE_CANCELLED") {
      setTimeout(() => {
        toast.error("Goggle login Cancelled");
      }, 100);
      // clean URL
      window.history.replaceState({}, document.title, "/auth/login");
    }
    ...
    <>
      <div className="max-w-md w-full mx-auto border border-gray-300 rounded p-5  bg-white">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-sm text-gray-600">Login to continue to Medicare Hospital</p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-4">
            <div>
              ...
              />
              {errors.login_id && <span className="text-sm text-red-600">{errors.login_id}</span>}
            </div>
      ...
      ${loading ? "opacity-60 cursor-not-allowed" : "cursor-pointer"}`}
          ...
          <p className="text-slate-600 text-sm mt-6 text-center">
            ...
            <NavLink to="../register" className="text-blue-600 font-medium hover:underline ml-1">
              Register now
            </NavLink>
          </p>
        </form>
